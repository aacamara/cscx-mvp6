 ...
 *
 * Types for analyzing integration/API usage data and calculating
 * technical health scores with issue detection and recommendations.
 */
...
export interface UploadIntegrationDataRequest {
  customerId: string;
  customerName?: string;
  fileContent: string;
  fileType: 'json' | 'csv';
...
export interface UploadIntegrationDataResponse {
  success: boolean;
  data: {
    uploadId: string;
    customerId: string;
    customerName: string;
    summary: ParsedIntegrationData['summary'];
    dateRange: ParsedIntegrationData['dateRange'];
    parseErrors: string[];
  };
}
...
export interface ScheduleCallRequest {
  ...
  additionalAgendaItems?: string[];
}

// ============================================
// UPLOAD STATE TYPES (for frontend)
// ============================================

export type UploadStatus = 'idle' | 'uploading' | 'parsing' | 'analyzing' | 'complete' | 'error';

export interface TechnicalHealthUploadState {
  status: UploadStatus;
  progress: number;
  uploadId?: string;
  summary?: ParsedIntegrationData['summary'];
...
export interface TechnicalHealthFilters {
  integrationTypes?: IntegrationDataType[];
...
export interface TechnicalHealthHistory {
  date: string;
  score: number;
  components: TechnicalHealthComponents;
  issueCount: number;
